...
export const loader = async () => {
  ...
  return data.map((country: any) => ({
    ...
    openStreetMapId: country.maps?.openStreetMap,
  }));
}
      ...
      <section className="mt-2.5 wrapper-md">
        <form className="trip-form" onSubmit={handleSubmit}>
          ...
          <footer className="px-6 w-full">
            <ButtonComponent type="submit" cssClass="button-class !h-12 !w-full" disabled={loading}>
              <img src={`/assets/icons/${loading ? 'loader.svg' : 'magic-star.svg'}`} alt="Create"
                   className={cn("size-5", {'animate-spin': loading})}/>
              <span className="p-18-semibold text-white">{loading ? 'Generation...' : 'Generate Trip'}</span>
            </ButtonComponent>
          </footer>
        </form>
      </section>
